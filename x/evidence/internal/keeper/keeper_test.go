...
import (
	...
	"github.com/evoblockchain/evochain/x/evidence/exported"
	"github.com/evoblockchain/evochain/x/evidence/internal/keeper"
	"github.com/evoblockchain/evochain/x/evidence/internal/types"
...
func MakeEVOApp() *app.EVOChainApp {
	genesisState := app.NewDefaultGenesisState()
	db := dbm.NewMemDB()
	evoblockchainapp := app.NewEVOChainApp(log.NewTMLogger(log.NewSyncWriter(os.Stdout)), db, nil, true, map[int64]bool{}, 0)

	stateBytes, err := codec.MarshalJSONIndent(evoblockchainapp.Codec(), genesisState)
	if err != nil {
		panic(err)
	}
	evoblockchainapp.InitChain(
		abci.RequestInitChain{
			Validators:    []abci.ValidatorUpdate{},
			AppStateBytes: stateBytes,
		},
	)
	return evoblockchainapp
}
