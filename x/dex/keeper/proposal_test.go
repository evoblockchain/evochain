...
import (
	...
	ordertypes "github.com/evoblockchain/evochain/x/order/types"
...
func TestKeeper_CheckMsgSubmitProposal(t *testing.T) {
	...
	content := types.NewDelistProposal("delist xxb_evo", "delist asset from dex", tokenPair.Owner, tokenPair.BaseAssetSymbol, tokenPair.QuoteAssetSymbol)
	...
	// error case: fail to check proposal because initial deposit must not be less than 100.00000000evo
...
func TestKeeper_AfterSubmitProposalHandler(t *testing.T) {
	testInput := createTestInputWithBalance(t, 1, 10000)
	ctx := testInput.Ctx
	content := types.NewDelistProposal("delist xxb_evo", "delist asset from dex", nil, "", "")
	proposal := govTypes.Proposal{Content: content}
...
func TestKeeper_VoteHandler(t *testing.T) {
	...
	content := types.NewDelistProposal("delist xxb_evo", "delist asset from dex", nil, tokenPair.BaseAssetSymbol, tokenPair.QuoteAssetSymbol)
	proposal := govTypes.Proposal{Content: content}
