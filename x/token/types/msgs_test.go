...
func TestNewMsgTokenBurn(t *testing.T) {
	...
	testCase := []struct {
		...
		{NewMsgTokenBurn(decCoin1, addr), common.ErrInsufficientCoins(DefaultParamspace, "100.0000000000000000001evo-ads")},
	}
...
func TestNewTokenMsgSend(t *testing.T) {
	...
	Errorcoins := sdk.SysCoins{
		sdk.NewDecCoinFromDec("evo", sdk.NewDec(100)),
		sdk.NewDecCoinFromDec("evo", sdk.NewDec(100)),
	...
	testCase := []struct {
		...
		{NewMsgTokenSend(fromAddr, toAddr, Errorcoins), ErrInvalidCoins("100.000000000000000000evo,100.000000000000000000evo,100.000000000000000000oke")},
...
func TestNewTokenMultiSend(t *testing.T) {
	...
	// correct message
	coinStr := `[{"to":"evo16zra6y26jytss520xrffq96p4fp4mnantcvyp2","amount":"1` + common.NativeToken + `"}]`
	transfers, err := StrToTransfers(coinStr)
	require.Nil(t, err)

	// coins not positive
	toAddr0, err := sdk.AccAddressFromBech32("evo16zra6y26jytss520xrffq96p4fp4mnantcvyp2")
	require.Nil(t, err)
	...
	testCase := []struct {
		...
		{NewMsgMultiSend(fromAddr, transfers0), ErrInvalidCoins("0.000000000000000000evo")},
...
func TestNewMsgTransferOwnership(t *testing.T) {
	...
	testCase := []struct {
		...
		{NewMsgTransferOwnership(fromAddr, toAddr, "1evo-ads"), ErrConfirmOwnershipNotExistOrBlockTimeAfter()},
	}
	for _, msgCase := range testCase {
