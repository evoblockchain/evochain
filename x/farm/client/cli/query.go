...
Example:
$ %s query farm rewards pool-eth-xxb evo1ss60jyagpw8v224006cqdkw22m32ytz54av3h3
`,
				version.ClientName,
			),
		),
...
func GetCmdQueryWhitelist(storeName string, cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		Use:   "whitelist",
		Short: "query the whitelist of pools to farm evo",
		Long: strings.TrimSpace(
			fmt.Sprintf(`Query the current whitelist of pools which are approved to farm evo.
...
Example:
$ %s query farm account evo1ss60jyagpw8v224006cqdkw22m32ytz54av3h3
`,
				version.ClientName,
			),
		),
...
Example:
$ %s query farm lock-info pool-eth-xxb evo1ss60jyagpw8v224006cqdkw22m32ytz54av3h3 
`,
				version.ClientName,
			),
		),
