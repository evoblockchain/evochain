...
func GetCmdDestroyValidator(cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		...
		Args:  cobra.NoArgs,
		Short: "deregister the validator from the EvoChain and unbond the min self delegation",
		Long: strings.TrimSpace(
			fmt.Sprintf(`Deregister the validator from the EvoChain and unbond the min self delegation.
...
Example:
$ %s tx staking proxy bind evo1ss60jyagpw8v224006cqdkw22m32ytz54av3h3 --from mykey
`,
				version.ClientName),
		),
		RunE: func(cmd *cobra.Command, args []string) error {
