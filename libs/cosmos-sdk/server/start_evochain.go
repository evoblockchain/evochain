...
import (
	...
	mpttypes "github.com/evoblockchain/evochain/libs/cosmos-sdk/store/mpt/types"
	sdkstoretypes "github.com/evoblockchain/evochain/libs/cosmos-sdk/store/types"
	...
	cmn "github.com/evoblockchain/evochain/libs/tendermint/libs/os"
	"github.com/evoblockchain/evochain/libs/tendermint/state"
	tmtypes "github.com/evoblockchain/evochain/libs/tendermint/types"
	evmtypes "github.com/evoblockchain/evochain/x/evm/types"
)

// evochain full-node start flags
const (
...
func StopCmd(ctx *Context) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				errStr := fmt.Sprintf("%s Please finish the process of evochaind through kill -2 pid to stop gracefully", err.Error())
				cmn.Exit(errStr)
			}
			defer f.Close()
...
func RegisterServerFlags(cmd *cobra.Command) *cobra.Command {
	...
	cmd.Flags().String(FlagUlockKeyHome, os.ExpandEnv("$HOME/.evochaincli"), "The keybase home path")
	cmd.Flags().String(FlagRestPathPrefix, "evochain", "Path prefix for registering rest api route.")
...
func nodeModeCmd(ctx *Context) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "node-mode",
		Short: "evochaind start --node-mode help info",
		Long: `There are three node modes that can be set when the evochaind start
