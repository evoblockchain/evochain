...
import (
	...
	cosmos_context "github.com/evoblockchain/evochain/libs/cosmos-sdk/client/context"
	"github.com/evoblockchain/evochain/libs/cosmos-sdk/client/flags"
	cmserver "github.com/evoblockchain/evochain/libs/cosmos-sdk/server"
	"github.com/evoblockchain/evochain/libs/cosmos-sdk/store/mpt"
	cosmost "github.com/evoblockchain/evochain/libs/cosmos-sdk/store/types"
	...
	"github.com/evoblockchain/evochain/app/rpc/types"
...
const (
	...
	defaultChainID         = 65
	defaultMinGasPrice     = "0.0000000001evo"
...
func (suite *RPCTestSuite) SetupTest() {
	...
	// set evochaincli path
	cliDir, err := ioutil.TempDir("", ".evochaincli")
	if err != nil {
		panic(err)
	}
	...
	// set evochaind path
	serverDir, err := ioutil.TempDir("", ".evochaind")
	if err != nil {
		panic(err)
	}
	...
	chainId := apptesting.GetEVOhainID(1)
	...
	//info, err := Kb.CreateAccount("account1", "puzzle glide follow cruel say burst deliver wild tragic galaxy lumber offer", "", "12345678", "m/44'/60'/0'/0/1", "eth_secp256k1")
...
func (suite *RPCTestSuite) TestEth_GetBalance() {
	// initial balance of hexAddr2 is 1000000000evo in test.sh
...
func (suite *RPCTestSuite) TestEth_GasPrice() {
	...
	// min gas price in test.sh is "0.000000001evo"
...
func (suite *RPCTestSuite) TestEth_SendTransaction_Transfer() {
	...
	suite.Require().NotNil(receipt)
	suite.Require().Equal("0x1", receipt["status"].(string))
	//suite.T().Logf("%s transfers %sevo to %s successfully\n", hexAddr1.Hex(), value.String(), receiverAddr.Hex())
	...
	//suite.Require().NotNil(receipt)
	//suite.Require().Equal("0x1", receipt["status"].(string))
	//suite.T().Logf("%s transfers %sevo to %s successfully with nil gas price \n", hexAddr1.Hex(), value.String(), receiverAddr.Hex())
...
func (suite *RPCTestSuite) TestEth_GetBlockByHash() {
	...
	// TODO: EVOChain only supports the block query with txs' hash inside no matter what the second bool argument is.
