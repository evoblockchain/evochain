...
import (
	...
	cosmossdk "github.com/evoblockchain/evochain/libs/cosmos-sdk/types"
	...
	tendertypes "github.com/evoblockchain/evochain/libs/tendermint/types"
	"github.com/evoblockchain/evochain/x/distribution/keeper"
...
func TestEVOChainAppExport(t *testing.T) {
	db := dbm.NewMemDB()
	app := NewEVOChainApp(log.NewTMLogger(log.NewSyncWriter(os.Stdout)), db, nil, true, map[int64]bool{}, 0)
	...
	app2 := NewEVOChainApp(log.NewTMLogger(log.NewSyncWriter(os.Stdout)), db, nil, true, map[int64]bool{}, 0)
...
func (suite *FakeBlockTxTestSuite) TestFakeBlockTx() {
	testCases := []struct {
		...
		},
		{
			...
			5, //insufficient funds: insufficient funds to pay for fees; 890.000000000000000000evo < 1000000000000000000.000000000000000000evo
			0,
		},
		{
