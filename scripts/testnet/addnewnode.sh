...
if [ -d ${EVOHAIN_NET_CACHE}/node0/evochaind ]; then
    seed_addr=$(${BIN_NAME} tendermint show-node-id --home ${EVOHAIN_NET_CACHE}/node0/evochaind)@${IP}:${seedp2pport}
fi
...
init() {
    ...
    if [ -d ${EVOHAIN_NET_CACHE}/${NAME} ]; then
        echo "Invalid index!"
        echo "<${EVOHAIN_NET_CACHE}/${NAME}> already exists. Use '-n' to try another index."
    ...
    ${BIN_NAME} init ${NAME} -o --chain-id ${CHAIN_ID} --home ${EVOHAIN_NET_CACHE}/${NAME}/evochaind --node-index ${INPUT_INDEX}
}
...
start() {
    ...
    rm ${EVOHAIN_NET_CACHE}/${NAME}/evochaind/config/genesis.json
    cp ${EVOHAIN_NET_CACHE}/node0/evochaind/config/genesis.json ${EVOHAIN_NET_CACHE}/${NAME}/evochaind/config/
...
#     echo "${BIN_NAME} --home ${EVOHAIN_NET_CACHE}/${NAME}/evochaind  start --p2p.laddr tcp://${IP}:${p2pport} --p2p.seeds ${seednode} --rpc.laddr tcp://${IP}:${rpcport}"
...
#    LOG_LEVEL=main:info,*:error,state:debug,consensus:debug
    ...
    ${BIN_NAME} start \
    --chain-id ${CHAIN_ID} \
    --home ${EVOHAIN_NET_CACHE}/${NAME}/evochaind \
    ...
    --rpc.laddr tcp://${IP}:${rpcport} > ${EVOHAIN_NET_CACHE}/rpc${INPUT_INDEX}.log 2>&1 &
